#Here's a hard-selection version of the Leven 1953 model of course-grained selection over space
...
#With hard selection a patch's contribution to the dispersal pool is a function of the larvae that
#settle their and their within-patch fitness. These zis are the proportions of larvar that come in to a patch
zis <- c(0.4,0.05,0.5,0.05)
#cis <- c(0.4,0.05,0.5,0.05)

#And here's a function to calculate the cis, that is the proportion of disersers contributed by each patch
get.cis <- function(p, vm, zis){
    q <- 1-p
    Average.Local.Fitnesses <- numeric(length=4)
    Global.Fitness <- 0
    for (i in 1:4){
        Vi <- vm[i,1]
        Wi <- vm[i,3]
        ...
        Average.Local.Fitnesses[i] <- Wi.bar
        Global.Fitness <- Global.Fitness + (Wi.bar*zi)
    }
    cis <- numeric(length=4)
    for (i in 1:4){
        w <- Average.Local.Fitnesses[i]
        z <- zis[i]
        ci <- zi*w
        cis[i] <- ci
    }
...
#the ci terms
#Note: this is the same as in the soft-selection version
get.global.delta <- function(p, vm, cis){
    ...
    for (i in 1:4){ #for each resource patch
        ...
        ci <- cis[i] #get the proportion of emmigrants from the patch
        Wi.bar <- (p^2)*Vi + 2*p*q + (q^2)*Wi # this is average local viability
...
for (i in 1:length(ps)){
    p1 <- ps[i]
    cis <- get.cis(p1, vm, zis)
    gd <- get.global.delta(p1, vm, cis)
    gds[i] <- gd
}

plot(gds~ ps, type='l', lwd=3, col='red', ylab='Change in S', xlab='Frequency of allele S', main='Hard selection in a mixed environment')
